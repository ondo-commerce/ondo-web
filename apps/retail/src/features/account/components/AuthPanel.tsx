...
/**
 * 계정 4화면의 흰 카드. 제목 한 줄 + 보조 설명 한 줄 + 본문.
 *
 * `Panel`을 그대로 쓰지 않고 감싸는 이유는 두 가지다.
 * 1. 안쪽 여백이 28px이다 (`Panel` 기본은 16px). 로그인 전 화면은 카드 하나가
 *    화면 전부라 셸 안의 패널보다 넉넉하다.
...
export function AuthPanel({
  ...
  return (
    <Panel className="p-7">
      ...
      {lead ? (
        <p className="text-muted-foreground mt-1.5 text-body">{lead}</p>
      ) : null}
      {children}
    </Panel>
  );
...
export function AuthLinks({ children }: { children: ReactNode }) {
  return (
    <div className="text-muted-foreground mt-4 flex items-center justify-center gap-2.5 text-body">
      {children}
    </div>
  );
}
...
export function AuthSection({
  ...
  return (
    <section className={cn("mt-6", className)}>
      {title ? (
        <h2 className="text-muted-foreground mb-2.5 text-body">{title}</h2>
      ) : null}
      {children}
    </section>
  );
