 ...
 * 하는 건 이 다섯뿐이다. `notFound`는 에러 패널이 아니라 빈 상태로 그리고,
 * `forbidden`은 다시 시도해도 같아서 버튼을 빼고, 나머지는 재시도를 준다.
 */
 ...
 *
 * `ApiError`가 아닌 것(코드 버그·렌더 에러)도 받는다. 경계는 쿼리 에러만 잡는 게
 * 아니라 그 아래에서 터진 전부를 잡기 때문이다.
 */
...
function kindOf(code: string, status: number): ErrorKind {
  switch (code) {
    case TRANSPORT_ERROR_CODE.NETWORK:
      return "network";
    case WHOLESALE_ERROR_CODE.RESOURCE_NOT_FOUND:
      return "notFound";
    case WHOLESALE_ERROR_CODE.ACCESS_DENIED:
    case WHOLESALE_ERROR_CODE.NOT_APPROVED:
      return "forbidden";
    case WHOLESALE_ERROR_CODE.INTERNAL_ERROR:
      return "server";
  }
