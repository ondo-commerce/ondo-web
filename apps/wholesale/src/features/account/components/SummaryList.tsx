 ...
 *
 * 간격을 빼면 값이 라벨 바로 옆에 붙어 두 열이 한 덩어리로 읽힌다 — 원본 `.dl`은
 * `gap:9px 24px`이라 **값의 시작점이 라벨 시작점에서 144px**이다
 * (`retail-account` F4가 이 간격이 0이라 걸렸다).
 *
 * 값에 `tabular-nums`를 주는 이유: 사업자 등록번호와 신청 일시가 세로로 붙어
