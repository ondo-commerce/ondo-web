 ...
 *
 * 실행 버튼 네 개는 누르는 순간 라우트가 바뀌면서 버튼 자체가 사라진다. 그때
 * 포커스는 `<body>`로 떨어지고 도착 화면은 아무 말도 하지 않았다 — 키보드·낭독기
 * 사용자는 실행이 끝났는지 모른 채 문서 맨 앞에서 다시 Tab을 시작했다
 * (`wholesale-account` F5). 다이얼로그 쪽은 `useReturnFocus`가 "연 자리로
 * 되돌린다"로 풀지만, 실행 버튼은 되돌아갈 자리가 사라지므로 반대로
 * **도착 지점이 받는다.**
 *
