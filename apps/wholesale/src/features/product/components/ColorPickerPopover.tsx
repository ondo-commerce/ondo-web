...
import { useColorsQuery } from "../api/queries";
import { flattenColors } from "../derive";
import type { ColorItem } from "../types";
import { QueryBoundary } from "@/shared/api/QueryBoundary";

/**
 * 색상 마스터 팝오버 — `GET /colors`의 그룹 → 색상 2단. 자유 입력을 받지 않는다.
 * 상품에 넣을 수 있는 색은 이 목록이 전부다(스펙).
 *
...
export function ColorPickerPopover({
  ...
  /** 지금 골라져 있는 색 id들. 팝오버를 열 때 초안의 출발점이 된다 */
  selected: number[];
  onConfirm: (colors: ColorItem[]) => void;
  ...
  return (
    <Popover open={open} onOpenChange={openChange}>
      ...
      >
        {/* 팔레트가 서버에서 오므로 팝오버 안이 경계다. 실패해도 폼은 살아 있다 */}
        <QueryBoundary className="min-w-64">
          <Palette
            draft={draft}
            onToggle={toggle}
            onReset={() => setDraft([])}
...
function Palette({
  ...
  const { data: groups } = useColorsQuery();
  ...
  return (
    <>
      ...
      <div className="mt-6 flex shrink-0 items-center justify-end gap-2">
        <Button variant="soft" onClick={onReset} disabled={draft.length === 0}>
          초기화
        </Button>
        <Button
          onClick={() =>
            /* 팔레트 순서를 유지한 채 초안 id를 색으로 되돌린다 */
            onConfirm(flattenColors(groups).filter((c) => draft.includes(c.id)))
          }
        >
          확인 ({draft.length})
        </Button>
