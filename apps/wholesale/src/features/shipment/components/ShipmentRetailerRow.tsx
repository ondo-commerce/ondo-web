 ...
 * 건수의 단위(대기 줄 ↔ 봉투)와 펼친 본문뿐이라, 단계마다 행을 따로 두면
 * 같은 셸이 세 벌이 된다. 이 행은 단계를 모른다.
 *
 * 껍데기(chevron 열·확장행·열 폭 규칙)는 `Table.ExpandableRow`가 갖고 있다 —
 * 주문 탭과 같은 한 벌이다.
 *
 * 두 숫자를 열로 갈라 세우면 소매처끼리 크기가 세로로 비교된다 — 어디부터 포장할지 고르는
 * 화면이라 그 비교가 이 목록에서 실제로 하는 일이다.
 */
export function ShipmentRetailerRow({
  ...
  /** 건수. 단위는 단계가 정한다 — 포장 대기는 대기 줄, 나머지는 봉투다 */
  count: number;
  /** 총 수량. 단계와 무관하게 낱개 수다 */
  qty: number;
  open: boolean;
