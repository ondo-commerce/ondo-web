...
| PR 성격                                                                  | 규칙                                                                                   |
| ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------- |
| 일반 기능/버그                                                           | **CI green + AI 리뷰 1회(지적사항 확인) + PR 템플릿 셀프 체크 완료 → 셀프 머지 OK**    |
| 아키텍처 변경 (폴더 구조, 상태관리, 라우팅 규칙, **`dependencies` 추가**) | **ADR 링크 필수 + BE 1명 이상 승인**. 코드가 아니라 "결정"을 리뷰받는 것               |
| **`devDependencies` 추가** (lint·훅·빌드 도구처럼 번들에 안 실리는 것)   | 셀프 머지 OK. 단 **PR 본문에 사유와 되돌리기 비용을 적는다**                           |
| API 연동 PR                                                              | BE 담당자를 **reviewer가 아니라 mention**으로 태그 (계약 어긋남 조기 발견)             |

**의존성 기준을 둘로 나누는 이유.** `dependencies`는 번들에 실려 사용자에게 배포되고, 빼려면 그걸 쓰는 코드를 전부 걷어내야 한다. `devDependencies`는 개발 환경에만 있고 되돌리는 비용이 몇 분이다 — #10의 husky·lint-staged가 그 사례였다. BE가 이해관계 없는 FE 커밋 훅에까지 승인을 요구하면 규칙을 어기는 습관만 생긴다.

**AI 리뷰 게이트 — 무엇을 대체하고 무엇을 못 하나**

- 대체하는 것: **셀프 리뷰 체크리스트** (PR 템플릿의 "셀프 리뷰 체크" 절). 자기 코드를 자기가 봐주는 항목은 AI가 더 꼼꼼하다. 지적은 반영하거나, 안 고치면 그 이유를 답글로 남긴다
- 대체하지 못하는 것: **BE 승인.** "이 결정이 6개월 계획에 맞나", "BE와의 계약이 이걸 허용하나"는 AI가 판단할 수 없다. 아키텍처 변경은 여전히 사람이 승인한다
- **리뷰하는 AI는 개발한 AI와 독립이어야 한다.** 같은 대화 맥락을 들고 리뷰하면 개발할 때 놓친 것을 리뷰에서도 똑같이 놓친다. 그래서 개발에 쓴 세션에서 리뷰를 돌리지 않고, PR에 자동으로 붙는 리뷰 앱이 맡는다 (`.coderabbit.yaml`)
...
- **`config` = `packages/config` 패키지 + 레포 루트 설정·CI·문서** (`docs/`, `.github/`, `commitlint.config.cjs`, `turbo.json` 등). 레포 전역 변경용 scope를 따로 두지 않고 `config`가 둘 다 담기로 했다 (#12)
...
**이슈·PR은 화면 1장 단위로 잡는다.** 새 feature 폴더가 800줄을 넘긴 네 건 중 둘은 화면 여러 장을 한 이슈에 묶어서였다 — 규칙이 아니라 스코프 문제다 (#142). 한 화면이 그래도 800줄을 넘으면 쪼갤 곳이 없으니, **사유와 파일 구성(파일별 역할)을 PR 본문에 적고** 올린다. 숫자는 올리지 않는다 — 기존 코드를 고치는 PR엔 800줄이 여전히 맞는 상한이다.
